...
export abstract class DirectQueryList<T>
  ...
  displayCount(): string {
    return `${this.page * this.pageSize + 1} - ${Math.min(
      this.filteredItems.length,
      this.page * this.pageSize + this.pageSize
    )}${this.allItemsFetched ? ` of ${this.filteredItems.length}` : ""}`;
  }
  ...
  protected listDisplayItems(): void {
    ...
    this.displayItems = this.filteredItems.slice(
      startIndex,
  ...
  protected subscribeToPaging(): void {
    this.paginatorSub = this.paginator.page.subscribe(() => {
      ...
      if (!this.allItemsFetched && this.filteredItems.length <= last) {
        this.fetchItems(
          first,
          Math.min(last - this.filteredItems.length, this.pageSize)
        ).subscribe(() => {
