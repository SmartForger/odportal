...
export class ListAllAppsComponent implements OnInit, OnDestroy, OnChanges {
  ...
  isAllSelected() {
	...
	private filterItems() {
		...
		const titleFilter = (app: App) => !appTitle || app.appTitle.toLowerCase().indexOf(appTitle) >= 0;
		...
		this.filteredItems = this.allItems.filter((app) => titleFilter(app) && statusFilter(app) && typeFilter(app));
	...
	private displayConfirmModal(config: PlatformModalModel, apps: App[], callback: () => void) {
		...
		let modalRef: MatDialogRef<PlatformModalComponent> = this.dialog.open(PlatformModalComponent, { data: config });
