...
export class ListPendingUsersComponent extends ListUsersBaseComponent {
  ...
  constructor(
    ...
    this.query = function (first: number, max: number) {
      return this.roleService.listUsers(
  ...
  ngOnInit() {
    ...
    this.roleService.list().subscribe((roles: Array<Role>) => {
      ...
      this.APPROVE_ROLE =
        roles.find(
          (role) => role.id === this.authSvc.globalConfig.approvedRoleId
        ) || this.emptyRole();
      this.PENDING_ROLE =
        roles.find(
          (role) => role.id === this.authSvc.globalConfig.pendingRoleId
        ) || this.emptyRole();
    });
  ...
  approve(user: UserProfileKeycloak): void {
    ...
    let modalRef: MatDialogRef<RealmRolePickerComponent> = this.dialog.open(
  ...
  deny(user: UserProfileKeycloak): void {
    ...
    let dialogRef: MatDialogRef<PlatformModalComponent> = this.dialog.open(
    ...
    this.userService.delete(this.activeUser.id).subscribe(
      ...
      (err: any) => {
        this.notificationsSvc.notify({
          ...
          message:
            "There was a problem while denying " + this.activeUser.username,
        });
      }
    );
  }
  ...
  private removeUser(user: UserProfileKeycloak): void {
    let index: number = this.items.findIndex(
      (u: UserProfileKeycloak) => u.id === user.id
    );
