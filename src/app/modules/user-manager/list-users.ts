...
export class ListUsersBaseComponent extends DirectQueryList<UserProfileKeycloak>
  implements OnInit {
  ...
  viewAttributes(user: UserProfileKeycloak): void {
    ...
    let modalRef: MatDialogRef<ViewAttributesComponent> = this.dialog.open(
      ViewAttributesComponent
    );
    modalRef.componentInstance.user = user;
    modalRef.componentInstance.close.subscribe((close: any) =>
      modalRef.close()
    );
  }
  ...
  assignRolesToSelectedUsers() {
    let modalRef: MatDialogRef<AssignRolesDialogComponent> = this.dialog.open(
    ...
    modalRef.afterClosed().subscribe((data) => {
      if (data) {
        ...
        const observables = this.selectionSvc
          .getSelectedItems()
          .map((user: UserProfileKeycloak) =>
            this.userService.addComposites(user.id, roles).pipe(
              map((response) => response),
              catchError((err) => of(new Error("error occured")))
            )
          );
        ...
        concat(...observables)
          ...
          .subscribe((result: any[]) => {
            ...
            let message = `Roles were added successfully to ${this.pluralize(
              result.length - failed
            )}`;
            if (failed > 0) {
              message += ` but ${this.pluralize(failed)} failed`;
    ...
    let modalRef: MatDialogRef<PlatformModalComponent> = this.dialog.open(
      PlatformModalComponent,
      {
        data: {
          ...
          title: "Suspend users",
          subtitle:
            "Are you sure you want to disable selected users and revoke log-in privileges?",
  ...
  protected listDisplayItems(): void {
    ...
    const userIds = this.displayItems
      .filter((u) => !u.vendorId)
      .map((u) => u.id);
    if (userIds.length === 0) {
      return;
  ...
  protected removeDeletedItems(result: any[]): void {
    ...
    this.items = this.items.filter(
      (item) => deletedUserIds.indexOf(item.id) < 0
    );
    this.filterUsers(this.search);
    this.selectionSvc.resetSelection();
  }
