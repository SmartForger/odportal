...
export class ListUsersComponent implements OnInit, OnDestroy {
  ...
  private subscribeToSessionUpdate(): void {
    this.sessionUpdatedSub = this.authSvc
      .observeUserSessionUpdates()
  ...
  addButtonClicked(): void {
    let modalRef: MatDialogRef<CreateUserFormComponent> = this.dialog.open(
    ...
    modalRef
      .afterOpened()
      .subscribe((open) => modalRef.componentInstance.clearForm());
  ...
  private removePendingRole(userId: string): void {
    const rolesToRemove: Array<Role> = this.roles.filter(
      (role: Role) => role.id === this.authSvc.globalConfig.pendingRoleId
    );
  ...
  private addActiveRole(userId: string): void {
    const rolesToAdd: Array<Role> = this.roles.filter(
      (role: Role) => role.id === this.authSvc.globalConfig.approvedRoleId
    );
  ...
  private sendRoleFailureNotification(): void {
    this.notifySvc.notify({
      type: NotificationType.Error,
      message:
        "The user was created, but there was a problem while flagging the user as approved",
    });
  }
