...
export class RegistrationManagerService {
    ...
    unapproveSection(regId: string, formId: string, sectionTitle: string): Observable<UserRegistration> {
        return this.http.patch<UserRegistration>(
            `${this.baseUri()}/registration/${regId}/form/${formId}/section/${sectionTitle}/unapprove`,
            null,
            {
                headers: this.authSvc.getAuthorizationHeader()
            }
